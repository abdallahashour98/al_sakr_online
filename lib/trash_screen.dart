  ...
  Future<void> _restore(String collection, String id) async {
    try {
      if (collection == 'sales') {
        ...
        await SalesService().restoreDeliveryOrder(id); // ✅ استرجاع الإذن
      } else {
    ...
    if (confirm) {
      try {
        if (collection == 'sales') {
          ...
          // أو يمكنك إضافة شروط PurchasesService().deletePurchaseForever(id) إذا كانت لديك
                  ...
                  if (collectionKey == 'delivery_orders') {
                    ...
                    if (collectionKey == 'sales') {
                      ...
                      secondParty = (s is List && s.isNotEmpty)
                          ? s[0]['name']
                          : (s is Map ? s['name'] : 'مورد غير معروف');
                    }
                    subtitleWidget = Row(
                      children: [
